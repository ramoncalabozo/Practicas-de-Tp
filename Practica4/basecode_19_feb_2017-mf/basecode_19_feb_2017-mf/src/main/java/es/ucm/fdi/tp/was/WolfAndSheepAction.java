...
public class WolfAndSheepAction implements GameAction <WolfAndSheepState,WolfAndSheepAction> {
	...
	private static final long serialVersionUID = -7061739732232203112L;// He puesto esto porque sino me dá un warning
	 ...
	 * Devuelve el jugador
	 */
	public int getPlayerNumber() { // Devuelve el turno del jugador
		return this.player;
	}
	
	@Override
	/**
	 * Aplica la jugada que le marca el estado que le viene como parámetro
	 */
	public WolfAndSheepState applyTo(WolfAndSheepState state) {
      ...
      /* } else if (TttState.isDraw(board)) { // Si hay empate
     ...
     if (next.getWinner()==-1){  
        if (next.movWolf()== 0){
        	 next = new WolfAndSheepState(state, board, true,1);// gana el lobo
        } else if (next.movSheep() == 0){
        	next = new WolfAndSheepState(state, board, true,0);
        } else if (next.estandetras()){
        	next = new WolfAndSheepState(state, board, true,0); // ganan las ovejas
        }
	}
        return next;
	}
	/**
	 * Devuelve la antigua columna
	 * @return
	 */
	public int getRow(){
		return this.oldrow; 
	...
	/**
	 * Método toString que nos sirve para escribier las jugadas que puede usar el usuario 
	 */
	public String toString(){
	 return "place " + player + " at (" + this.newrow + ", " + this.newcol + ")";// He puesto lo mismo que en el juego ttt,Para decir al usuario las opciones que tiene
	}
}
